...
def validation(fs, window_size, overlap_rate, input_axis, model_path, DataX, DataY, groups, augmentation_methods, plotsingal, ExValScoresDir):
    ...
    if plotsingal:
        plot_signal(X, y_predict_final, y_true_final, fs, ExValScoresDir)

    return scores


def plot_signal(X, y_predict_final, y_true_final, fs, ExValScoresDir):
...
def predict_data_unsupervised(X, model_path, window_size, overlap_rate):
    ...
    # predict y
    y_predict = cnn_model.predict(X_final).round()
