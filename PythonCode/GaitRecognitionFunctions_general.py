# -*- coding: utf-8 -*-
"""
Raw data: there are 17 labeled activities --> 0=none, 1=walking, 2=walking with transition, 3=shuffling,
...
V5 2 5: organized from V5 2 3-meeting-->
        1)Remove redundant words，
        2)try to improve the figure of true and predicted
        labels by modifying the ratio of down-sampling and overlap,
        3) see which activities wrongly classified as walking
        # copy the original y_data (contains all activities)
        # and also keep the copy y_train,y_test during the  split_rain_test, just like a shadow.
        # and we didn't run the model using the copy one
        direction: try LSTM model
"""
...
def load_matfiles(dataDir, aim_label, input_axis):
    """
    :param: Name of the folder where the data is located
            variable 'signal' in each .mat file ;
            0-10 columns in each signal: 0-2 ACCxyz;3-5 GYRxyz; 6-8 MAGxyz;9 activity_Labels,10, Time
            aim_label: 1, walking
    :function: load a list of .mat/csv data from a folder 'act17_mat'
    ...
    DataY_binary = np.concatenate(DataY_binary[:])
    groups = np.concatenate(groups[:])
    return DataX, DataY, DataY_binary, groups, filenames, subject_number
...
def segment_signal(X, y, groups, window_size, percentage):
    ...
    for (start, end) in windows(len(X), window_size, percentage):
        if len(X[start:end]) == window_size and len(np.unique(y[start:end])) == 1 and len(np.unique(groups[start:end])) == 1:
...
def segment_signal_unsupervised(X, groups, window_size, percentage):
    """ For a dataset without activity labels y and wanna to predict it
        segment signal X, and groups (subjects numbers) into windows
        For signal X, remove orientation per window
    :param window_size: seconds*sample rate
    :return: X: windows of signals (3acc, 3gyr)
             y: windows of labels
             Groups: windows of subjects number
             the function only remains the windows with one act label (DataY)
    """
...
def down_sampling_win(X, y, groups, i, output_path, data_type):
    ...
    for indx in np.unique(groups):
        ...
        pathToWinsize = os.path.join(output_path, 'windows_size')
        if not os.path.exists(pathToWinsize):
            os.makedirs(pathToWinsize)
        filepath = os.path.join(pathToWinsize, f'file_winsize_{data_type}_{i}.txt')
...
# %% 3.1 fit and evaluate a CNN model
...
def fit_and_evaluate(x_t, x_val, y_t, y_val,x_test,y_test, i, output_path):
    ...
    history = model.fit(x_t, y_t, epochs=epochs, batch_size=batch_size,verbose=verbose,validation_data=(x_val, y_val), callbacks=[early_stopping])
    ...
    score_test = model_performance(y_test, y_test_predict) # try to use from sklearn.metrics import classification_report
...
def model_performance(y_true, y_pred):
    ...
    spec = tn / (fp + tn)
    score = pd.DataFrame([[acc, preci, rec, f1, spec]],
                         columns=['Accuracy', 'Precision', 'Sensitivity', 'F1', 'Specificity'])
    return score


def run_model(X_train, X_val, y_train, y_val, groups_train, groups_val, X_test, y_test, groups_test, window_size, percentage, i,output_path):
    model_history = []

    # segment Xy data into windows and categorical y_data
    print("## 2) segment data into windows")
    X_train_win, y_train_win, groups_train_win = segment_signal(X_train, y_train, groups_train, window_size, percentage)
    X_val_win, y_val_win, groups_val_win = segment_signal(X_val, y_val, groups_val, window_size, percentage)
    X_test_win, y_test_win, groups_test_win = segment_signal(X_test, y_test, groups_test, window_size, percentage)
    ...
    X_train_blc, y_train_blc, groups_train_blc = down_sampling_win(X_train_win, y_train_win, groups_train_win, i, output_path, 'train')
    print("## 3.2) For validating data:")
    X_val_blc, y_val_blc, groups_val_blc = down_sampling_win(X_val_win, y_val_win, groups_val_win, i, output_path, 'vali')

    print("## 4) running the model")
    model, history, score_val, score_test = fit_and_evaluate(X_train_blc, X_val_blc, y_train_blc, y_val_blc, X_test_win, y_test_win, i, output_path)
